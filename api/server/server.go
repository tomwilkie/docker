...
func (s *Server) getNetworksJSON(eng *engine.Engine, version version.Version, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	if err := parseForm(r); err != nil {
		return err
	}
...
func (s *Server) deleteNetworks(eng *engine.Engine, version version.Version, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	if err := parseForm(r); err != nil {
		return err
	}
...
func (s *Server) postNetworkCreate(eng *engine.Engine, version version.Version, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	if err := parseForm(r); err != nil {
		return err
	}
	if err := checkForJson(r); err != nil {
...
func (s *Server) postNetworkUnplug(eng *engine.Engine, version version.Version, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	if err := parseForm(r); err != nil {
		return err
	}
...
func createRouter(s *Server, eng *engine.Engine) *mux.Router {
	...
	m := map[string]map[string]HttpApiFunc{
		"GET": {
			...
			"/networks/json":                  s.getNetworksJSON,
		},
		"POST": {
			"/auth":                         s.postAuth,
			"/commit":                       s.postCommit,
		...
		"DELETE": {
			...
			"/networks/{name:.*}":   s.deleteNetworks,
		},
		"OPTIONS": {
			"": s.optionsHandler,
		},
